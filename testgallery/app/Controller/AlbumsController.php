...
class AlbumsController extends AppController {
    ...
    public function userlogin(){
        ...
        $sendUserId = $this->User->checkUsernamePassword($data->username,$data->password,$data->type);
        echo json_encode($sendUserId);
    }

    public function addAlbum($userId){
        $this->layout = 'ajax';
        $this->autoRender = false;
        $data = json_decode(file_get_contents('php://input'));
        $sendAlbumData = $this->Album->addAlbum($data,$userId);
    ...
    public function listAlbum($userId){
        $this->layout = 'ajax';
        $this->autoRender = false;
        $sendAlbumList = $this->Album->getAlbum($userId);
    ...
    public function editAlbumName($albumId){
        $this->layout = 'ajax';
        $this->autoRender = false;
        $data = json_decode(file_get_contents('php://input'));
        $sendAlbumId = $this->Album->editAlbumName($data);
        echo $sendAlbumId;
    }
}